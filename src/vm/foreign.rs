...
impl ForeignFn {
    ...
    pub(in vm) fn call(&self, state: &mut State) -> Result<()> {
        ...
        let result = {
            ...
            let foreign = state.foreign_functions.get(&ff_key(&self.lib, &self.name))
                .unwrap();
            let code_ptr = CodePtr::from_ptr(*foreign as *const _);
            match self.return_type {
                ItemType::Int(_) =>
                    Val::Int(unsafe { call::<i32>(code_ptr, args.as_slice()) } as i64),
                ItemType::Char(_) =>
                    Val::Char(unsafe { call::<u8>(code_ptr, args.as_slice()) } as char),
                ItemType::Bool(_) =>
                    Val::Bool(unsafe { call::<i32>(code_ptr, args.as_slice()) } != 0),
                ItemType::Nil => {
    ...
    fn load_function(&self, state: &mut State) -> Result<()> {
        if !state.foreign_functions.contains_key(&ff_key(&self.lib, &self.name)) {
    ...
    fn load_handle(&self, state: &mut State) -> Result<()> {
        if !state.dl_handles.contains_key(&self.lib) {
            ...
            unsafe {
                ...
                if handle.is_null() {
                    return Err(format!("could not open dynamic library `{}`", &self.lib).into());
                }
